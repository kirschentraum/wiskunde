...
\begin{document}
	...
	\chapter{形式数学的描述（Description de la mathématique formelle）}
		...
		\section{逻辑理论（Théories logiques）}
			...
			\end{C}
			证明：根据公理模式\ref{Sch1}，$(A\text{或}A)\Rightarrow A$，根据公理模式\ref{Sch2}，$A\Rightarrow(A\text{或}A)$，根据证明规则6，$A\Rightarrow A$．
	...
	\chapter{集合论（Théorie des ensembles）}
		...
		\section{对应（Correspondances）}
			...
			\begin{cor}\label{cor31}
				...
				\par
				（5）$G_1$、$G_2$为图，且$G_1\subset G_2$，则p$r_1G_1\subset pr_1G_2$，$pr_2G_1\subset pr_2G_2$．
			\end{cor}
			证明：
			\par
			...
			\par
			（5）令$x$、$y$为不是常数的字母，由于$G_1\subset G_2$，因此$(x, y)\in G_1\Rightarrow (x, y)\in G_2$．
			\par
			...
			\begin{cor}\label{cor32}
				\hfill\par
				$(G, A, B)$为对应，$B\subset B'$，则$(G, A, B)$为对应．
			\end{cor}
			证明：根据定义可证．				
			...
			\end{theo}
			证明：根据定理\ref{theo12}、补充定理\ref{cor31}（5）、补充定理\ref{cor34}、显式公理\ref{ex1}可证．
			...
			\begin{de}
				...
				\par
				令$F$为$A$到$B'$的对应，其图为$G$，$F'$为$B$到$C$的对应，且$B'\subset B$，则$(G, A, B)$和$F'$的复合，也称为$F$和$F'$的复合，同样记作$F'\circ F$或者$F'F$．
			\end{de}
			注：原书对应的符合仅包含前半段定义，后半段是对原定义的推广．在《布尔巴基数学基础》的其他书中，会使用推广的定义．
			...
			\begin{cor}\label{cor63}
				\hfill\par
				$(G, A, B)$为函数，$B\subset B'$，则$(G, A, B')$为函数．
			\end{cor}
			证明：根据定义可证．
			...
			\begin{cor}\label{cor64}
				\textbf{函数的值的基本性质}
				\par
				（1）$F$为函数图，$x\in pr_1F$，则$(x, y)\in F$为$y$上的函数性公式．
				\par
				（2）$f$为函数，其图为$F$，则$(x\in pr_1F)\text{与}(y=f(x))\Leftrightarrow (x, y)\in F$．
				\par
				（3）$x\in pr_1F\Leftrightarrow (x, f(x))\in F$．
				\par
				（4）$x\in pr_1F\Rightarrow f(x)\in pr_2F$．
			\end{cor}
			证明：
			\par
			（1）根据定义可证．
			\par
			（2）如果$(x\in pr_1F)$，根据证明规则\ref{C46}、补充定理\ref{cor64}（1），$(y=f(x))\Leftrightarrow (x, y)\in F$．根据补充定理\ref{cor29}，$(x, y)\in F\Rightarrow (x\in pr_1F)$．得证．
			\par
			（3）根据补充定理\ref{cor64}（2）可证．
			\par
			（4）根据补充定理\ref{cor64}（3）、补充定理\ref{cor29}可证．
			
			\begin{cor}\label{cor65}
				\hfill\par
				$F$为函数图，$(\forall x)(x\in pr_1F\Rightarrow f(x)\in A)$，则$pr_2F\subset A$．
			\end{cor}
			证明：令$x$为不是常数的字母，由于$x\in pr_1F\Rightarrow f(x)\in A$，根据补充定理\ref{cor64}（2），$(x, y)\in F\Rightarrow (y=f(x))\text{与}(f(x)\in A)$，其等价于$(y=f(x))\text{与}(y\in A)$，因此$(\exists x)((x, y)\in F)\Rightarrow y\in A$，得证．
			...
			\end{cor}
			证明：根据补充定理\ref{cor64}（2），$(x\in pr_1F)\text{与}(y=f(x))\Leftrightarrow (x, y)\in F$，$(x\in pr_1G)\text{与}(y=f(x))\Leftrightarrow (x, y)\in G$．$(x, y)\in F\Rightarrow (x\in pr_1F)\text{与}y=g(x)$，因此$(x, y)\in F\Rightarrow (x, y)\in G$，同理可证$(x, y)\in G\Rightarrow (x, y)\in F$，根据补充定理\ref{cor27}（1），$F=G$．进而，如果$(f\text{的到达域})=(g\text{的到达域})$，根据定义，$f=g$．
			...
			证明：
			\par
			（1）令$f$的图为$F$，则$x\in f^{-1}\langle X \rangle \Leftrightarrow (\exists y)(y\in X\text{与}(x, y) \in F)$，根据补充定理\ref{cor64}（2），等价于$(\exists y)(y\in X\text{与}(x\in A)\text{与}(y=f(x)))$，等价于$(\exists y)(y=f(x))\text{与}x\in A\text{与}f(x) \in X$．$x\in A \Rightarrow (\exists y)((x, y)\in F)$，根据补充定理\ref{cor64}(2)，$x\in A \Rightarrow (\exists y)(y=f(x))$，因此$x\in f^{-1}\langle X \rangle \Leftrightarrow x\in A\text{与}f(x) \in X$．
			\par
			（2）由于$f$为$A$到$B$的映射，根据补充定理\ref{cor64}（4），$f(x)\in B$，根据补充定理\ref{cor70}（1），$x\in f^{-1}\langle B \rangle \\\Leftrightarrow x\in A$．
			\par
			（3）$x\in f^{-1}(y)$即$x\in f^{-1}\langle\{y\}\rangle$，根据补充定理\ref{cor70}（1）可证．
			\par
			（4）根据补充定理\ref{cor70}（3）可证．
			\par
			（5）令$f$的图为$F$，$f(x)\in f\langle X \rangle \Leftrightarrow (\exists z)(z\in X\text{与}(z, f(x))\in F)$．由于$x\in X$，因此$x\in A$，根据补充定理$\ref{cor64}$（3），$(x, f(x)) \in F$，因此$x\in X\text{与}(x, f(x)) \in F$，故$(\exists z)(z\in X\text{与}(z, f(x))\in F)$，得证．
			\par
			...
			\end{cor}
			证明：令$f$的图为$F$，根据补充定理\ref{cor64}（2），$(x\in A\text{与}y=f(x))\Leftrightarrow (x, y)\in F$，根据补充证明规则\ref{Ccor18}，$(x\in X\text{与}y=f(x))$为对于$x$、$y$生成图的公式．
			\par
			因此，$(x, y)\in G\Leftrightarrow x\in X\text{与}y=f(x)$，令$f$的图为$F$，根据补充定理\ref{cor64}（2），等价于$x\in X\text{与}(x, y)\in F$，则$pr_1G=\{x|x\in X\text{与}x\in A\}$，即$pr_1G=X$．对于$(x, y)\in G$、$(x, y')\in G$，有$(x, y)\in F$、$(x, y')\in F$，由于$F$为函数图，故$y=y'$，因此，$G$为函数图．
			...
			\end{cor}
			证明：令$f$的图为$F$，$f|X$的图为$G$，$x$、$y$为不是常数的字母．由于$(x\in X\text{与}y=f(x))\\\Rightarrow y=f(x)$，因此，$F\subset G$．根据补充定理\ref{cor64}（2）、补充证明规则\ref{Ccor16}（1），$(x\in X\text{与}y=f(x))\Leftrightarrow y=f|X(x)$，则$x\in X\Leftrightarrow f(x)=f|X(x)$，因此，$f|X$和$f$在$X$上重合．又因为$X$为$f|X$的定义域，因此，$f$为$f|X$在其定义域上的延拓．
			...
			证明：
			\par
			（1）设$x\in X$，根据补充定理\ref{cor64}（2），$(x, f(x))\in F$．令$f|X$的图为$G$，则$y=f(x)\Leftrightarrow (x, y)\in G$，即$y=f(x)\Leftrightarrow y=f|X(x)$，因此$f(x)=f|X(x)$．
			\par
			（2）$(x, z)\in F\circ \Delta_X\Leftrightarrow (\exists y)((x, y)\in \Delta_X\text{与}(y, z)\in F)$，根据补充定理\ref{cor59}，等价于$x\in X\text{与}(x, z)\in F$，根据补充定理\ref{cor64}（2），等价于$x\in X\text{与}y=f(x)$，得证．
			...
			\par
			根据补充定理\ref{cor64}（2），$y=F(x)\Leftrightarrow x\in A\text{与}y=T$．假设$x\in A$，则$y=F(x)\Leftrightarrow y=T$，因此$F(x)=T$．故$x\in A\Rightarrow F(x)=T$．
			\par
			...
			\par
			根据补充定理\ref{cor64}（2），$(\forall x)(\forall y)(\forall z)((y, x)\in F\text{与}(z, x)\in F\Rightarrow (y=z))\\\Leftrightarrow (\forall x)(\forall y)(\forall z)(y\in A\text{与}z\in A\text{与}x=f(y)\text{与}x=f (z)\Rightarrow (y=z))$．
			...
			\par
			令$f$的图为$F$，$g$的图为$G$，假设$(x, y)\in F$，则$y\in B$，$y=f(x)$．由于$g(f(x))=x$，因此$g(y)=x$，根据补充定理\ref{cor64}（2），$(y, x)\in G$．
			\par
		...
		\section{\texorpdfstring{集族的并集和交集（Réunion et intersection d'une \\famille d'ensembles）}{集族的并集和交集（Réunion et intersection d'une famille d'ensembles）}}
			...
			\end{cor}
			证明：设$f|X$的图为$G$，根据补充定理\ref{cor64}（2），$x\in X\text{与}y=f(x)\Leftrightarrow (x, y)\in G$，$x\in A\text{与}y=f(x)\Leftrightarrow (x, y)\in F$．根据补充定理\ref{cor23}（2），$x\in X\text{与}y\in B\Leftrightarrow (x, y)\in X\times B$．根据补充定理\ref{cor64}（4），$x\in A\Rightarrow f(x)\in B$．因此$(x, y)\in (F\cap X\times B)\Leftrightarrow x\in A\text{与}y=f(x)\text{与}x\in X\text{与}y\in B$，等价于$x\in X\text{与}y=f(x)$，得证．
		...
		\section{等价关系（Relations d'équivalence）}		
			...
			证明：
			\par
			（1）设$x\in C(a, b)$、$y\in C(a, b)$，则$(x|y)(a|x)R$、$R(x|y)(b|x)R$、$(a|x)R$、$(b|x)R$、$(a|x)(b|y)R$均为真，且$a\in E$、$b\in E$、$x\in E$、$y\in E$，根据定义，无论$a$、$b$、$x$、$y$中是否有相同的，$R$均为真．故$C(a, b)$关于$R$稳定．
			\par
			如果$x\in C(a, b)$、$y\in C(a, b)$并且$x\neq y$，则对任意$z\in C(x, y)$，则$z\in E$并且\\$(z|y)(a|x)R$、$(z|y)(b|x)R$，故$z\in C(a, b)$；反过来，同理可证对任意$z\in C(a, b)$，有$z\in C(x, y)$，因此$C(x, y)=C(a, b)$．
			\par
			对任意$x\in C(a, b)$，则$(x|y)(a|x)R$成立，反过来，如果$x$是$a$所在的$E$关于$R$的连通分量的元素，根据数学归纳法可证$x\in C(a, b)$，故$C(a, b)$是$a$所在的$E$关于$R$的连通分量．
			\par
			若$E$的两个不同组成部分的交集有两个元素$x$、$y$，则二者均为$C(x, y)$，矛盾，故最多有一个元素．假设$A\cap B$、$B\cap C$、$C\cap A$均不为空，设$A\cap B=\{a\}$，$B\cap C=\{b\}$，$C\cap A=\{c\}$，若其中$a=b$，则$a=c$，三者相同，若三者均不相同，则$A=C(a, c)$，$B=C(a, b)$，$C=C(b, c)$，因此$(b|y)(a|x)R$、$(a|y)(c|x)R$、$(c|y)(b|x)R$，因此$b\in A$、$c\in B$、$a\in C$，故$A\cap B$、$B\cap C$、$C\cap A$均有不少于三个公共元素，矛盾．得证．
			\par
			...
			\par
			（3）	
			对于具有$n-3$级非传递性的公式$R$，其组成部分的定义应为$C(a_1, a_2, \cdots a_{n-2})=\{x|x\in E\text{与}(x|y)(a_1|x)R\text{与}(x|y)(a_2|x)R\cdots\text{与}(x|y)(a_{n-2}|x)R\}$，其中对任意$i\in [1, n-2]$、$i\in [j, n-2]$，$(a_i|x)(a_j|y)R$均为真．
			\par
			充分必要条件为：
			\par
			第一，集族中任意两个集合的交集最多有$n-3$个元素；
			\par
			第二，集族中任意三个集合$A$、$B$、$C$，如果$A\cap B$、$B\cap C$、$C\cap A$均有$n-3$个元素，则$A\cap B$、$B\cap C$、$C\cap A$三者相同．
			\par
			如果上述性质成立，对任意元素$x$，考虑集族中有元素$x$的集合，去掉这些集合中的$x$后，得到的新集合根据归纳假设，具有$n-4$级非传递性，故充分性成立；
			\par
			如果$R$具有$n-3$级非传递性，类似习题\ref{exer76}（1）可以证明，$E$的任何两个组成部分的交集最多有$n-3$个元素，同时，设$A$、$B$、$C$均为$E$的组成部分，如果$A\cup B$有$n-3$个元素$a_1$、$a_2$、$\cdots$、$a_{n-3}$，同时$A\cup B\cup C$的元素数目小于$n-3$，则令$x\in A\cup C$同时$x\notin B$、$y\in B\cup C$同时$x\notin A$，进而，$x\in C(a_1, a_2, \cdots, a_{n-3}, y)$，因此$x\in B$，矛盾．故必要性成立．
			\par
			如果公式$R$具有$p$级非传递性，则集族任意两个元素的交集最多有$p$个元素，由于$q>p$，故$q$级非传递性的条件一成立；同时，如果集族中任意三个集合$A$、$B$、$C$，如果$A\cap B$、$B\cap C$、$C\cap A$均有$q$个元素，则令$M$有$q-p$个元素且$M\subset (A\cap B\cap C)$，令$A'=A-M$、$B'=B-M$、$C'=C-M$，则$A'\cap B'$、$B'\cap C'$、$C'\cap A'$均有$p-3$个元素，故全部$p-3$个元素均属于$A'\cap B'\cap C'$，也就是说，$q$级非传递性的条件二成立．综上，$R$具有$q$级非传递性．
			\par
	...
	\chapter{偏序集，基数，自然数（Ensembles ordonnés, cardinaux, nombres entiers）}
		...
		\section{自然数的运算（Calcul sur les entiers）}		
			...
			\begin{de}
				...
				\par
				则称$f$为$E$的子集$A$的特征函数，记作$\phi_A$．
			\end{de}
			
			\begin{cor}\label{cor314}
				\hfill\par
				对于$E$的子集$A$、$B$，$A=B\Leftrightarrow \phi_A=\phi_B$．
			\end{cor}
			证明：根据定义可证．
			...
			\begin{theo}\label{theo139}
				...
				对于$E$的子集$A$、$B$：
				\par
				（1）$\phi_E-A(x)=1-\phi_A(x)$．
				\par
				（2）$\phi_A\cap B(x)=\phi_A(x)\phi_B(x)$．
				\par
				（3）$\phi_A\cup B(x)+\phi_A\cap B(x)= \phi_A(x)+\phi_B(x)$．
			\end{theo}
			证明：根据定义可证．
			...
			\begin{theo}\label{theo142}
				\textbf{乘法原理}
				\par
				$a$、$b$为基数，$Card(E)=a$，$Card(F)=b$，$f$为$E$到$F$的满射，对任意$y\in F$，\\$Card(f^{-1}\langle y\rangle)=c$，则$a=bc$．
			\end{theo}
		...
		\section{无穷集合（Ensembles infinis）}		
			...
			\begin{de}
				...
				\par
				对任意序数$a>0$，如果单减有限序列$(b_i)_{i\in [1, k]}$，使$a=\sum\limits_{i\in [1, k]}{\omega_0}^{b_i}$，则称$(b_i)_{i\in [1, k]}$为$a$的展开，$b_1$为$a$的展开的最大指数，记作$\phi(a)$．
			\end{de}
			...
			\begin{cor}\label{cor354}
				...
				（1）$a<{\omega_0}^{\phi(a)+1}$．
				\par
				（2）如果$\phi(a)<\phi(b)$，则$a<b$．
				\par
				（3）如果$a<b$，则$\phi(a)\leq \phi(b)$．
				\par
			...
			\par
			（2）设$a$的展开有$n$项，则$a\leq {\omega_0}^{\phi(a)}n$，又因为$a<\omega_0\phi(a)+1$，因此$a<\omega_0\phi(b)$，故$a<b$．
			\par
			...
			\begin{cor}\label{cor358}
				...
				\par
				（2）$a$为无穷序数，则$a^{\omega_0}={\omega_0}^{\phi(a)\omega_0}$．
			\end{cor}
			证明：
			\par
			...
			\par
			（2）根据补充定理\ref{cor273}，$a^{\omega_0}\geq {\omega_0}^{\phi(a)\omega_0}$，$a^{\omega_0}\leq {\omega_0}^{(\phi(a)+1)\omega_0}$，根据补充定理\ref{cor351}可证．
			...
			\begin{cor}\label{cor360}
				\hfill\par
				$a>0$，则$\{x|x\text{为不可约的序数}\text{与}x\leq a\}$的最大元为${\omega_0}^{\phi(a)}$．
			\end{cor}
			...
			\end{cor}
			证明：如果$c$是$xy$的临界序数．由于${\omega_0}^{\phi(c)c}>c$，故$c={\omega_0}^{\phi(c)}$．根据补充定理\ref{cor263}，$\phi(c)$\\不可约，根据补充定理\ref{cor352}（1），存在序数$b$使$c$等于${\omega_0}^{{\omega_0}^b}$．
			\par
			反过来，如果存在序数$b$使$c$等于${\omega_0}^{{\omega_0}^b}$，根据补充定理\ref{cor352}（1），$\phi(c)$不可约；由于$a<c$时，$a<{\omega_0}^{\phi(a)+1}$，因此$ac\leq {\omega_0}^{\phi(a)+1+\phi(c)}$，又因为$\phi(a)+1<\phi(c)$，根据补充定理\ref{cor354}（4），$ac\leq c$，同时，由于$ac\geq c$，因此$ac=c$．
			...
			\end{cor}
			证明：
			如果对任意序数$a>1$、$a\leq c$，均存在$x$使$c=a^x$，则令$a={\omega_0}^{\phi(c)}$，故$c={\omega_0}^{\phi(c)x}$，因此$\phi(c)x =\phi(c)$，故$c={\omega_0}^{\phi(c)}$．
			\par
			如果$\phi(c)$可约：若$\phi(c)=a+b$，且$a>b$，则${\omega_0}^a<c$，${\omega_0}^{a2}>c$，矛盾；若$\phi(c)=a+b$，且$a<b$，则${\omega_0}^b<c$，${\omega_0}^{b2}>c$，矛盾；若$\phi(c)=2$，则$\omega_0+1<{\omega_0}^2$，$(\omega_0+1)^2>{\omega_0}^2$，矛盾；若$\phi(c)=a+a$，且$a>1$，则${\omega_0}^{a+1}<c$，${\omega_0}^{(a+1)2}>c$，矛盾．
			\par
			因此，$\phi(c)$不可约．根据补充定理\ref{cor352}（1），存在序数$b$使$\phi(c)= {\omega_0}^b$．
			\par
			反过来，如果$c={\omega_0}^{\phi(c)}$，$\phi(c)= {\omega_0}^b$，根据补充定理\ref{cor358}（1）、补充定理\ref{cor358}（2）、补充定理\ref{cor266}，对任意序数$a>1$、$a\leq c$，均存在$x$使$c=ax$．
			...
			\par
			如果$x_0\neq x_1$，则$(x_i)_{i\in N}$为严格单增序列．设$\phi(a)=b$，则对任意$i\in N$，如果$b<a$，则存在$x_i\geq b+1$，故$x_{i+1}\geq a$，因此$a=x_{i+1}$，故$a<x_{i+2}$，矛盾．因此$a=b$，即$a\geq {\omega_0}^a$．同时，根据补充定理\ref{cor275}，$a\leq {\omega_0}^a$，得证．
			\par
			...
			证明：
			\par
			根据定义可证$\omega_0$是$x^y$（$x\geq 2$、$y\geq 1$）的临界序数．令$a$为艾普塞朗数，$x<a$且$x\geq 2$，则$x^a\leq {\omega_0}^{(\phi(x)+1)a}$，由于$a$不可约，故$\phi(x)+1<a$，因此$x^a\leq {\omega_0}^a$，进而可得$x^a=a$．
			\par
	...
	\chapter{结构（Structures）}
		...
		\section{态射和派生结构（Morphismes et structures dérivées）}		
			\begin{STdef}
				...
				\par
				令$M$为比集合论强的理论，$X$是仅有一个基集合的结构种类．$\sigma$为项，字母$x$、$y$、$s$、$t$互不相同，且都不是$X$的代表特征和公理包含的字母．
				\par
